$(document).ready(function() {
    ...
    $("#addAddress").click(function () {
        let newField = $('<div>').addClass('data new-field')
            .append($('<input>').attr('type', 'text').attr('name', "address").attr('data-new', 'true'))
            .append($('<i>').addClass('fa fa-save save-icon').css('font-size', '18px'));
        addDeleteIcon(newField);
        $(this).before(newField);
    });

    $('.data').each(function() { //yha har .data element ke liye delete icon lag jayega using this(.data) by iterating each loop  
        addDeleteIcon($(this));
    });
